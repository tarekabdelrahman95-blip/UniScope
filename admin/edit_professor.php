...
// Get professor ID
$id = $_GET['id'];
$professor = $conn->query("SELECT * FROM professors WHERE professor_id = $id")->fetch_assoc();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE professors SET college_id=?, professor_name=?, department=? WHERE professor_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issi", $college_id, $name, $department, $id);
    
    if ($stmt->execute()) {
        header("Location: professors.php?msg=updated");
        exit();
    }
}
?>
...
<html>
<head>
    <title>Edit Professor - Admin</title>
    <style>
        ...
        <div class="form-container">
            <form method="POST">
                    ...
                    <select name="college_id" required>
                        <?php while($college = $colleges->fetch_assoc()): ?>
                            <option value="<?php echo $college['college_id']; ?>" 
                                <?php echo ($college['college_id'] == $professor['college_id']) ? 'selected' : ''; ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" value="<?php echo htmlspecialchars($professor['professor_name']); ?>" required>
                </div>
